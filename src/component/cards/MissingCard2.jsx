...
const MissingCard2 = ({ card }) => {
  ...
  return (
    ...
    className="bg-slate-50  sm:w-auto flex flex-col items-center gap-4 justify-center border border-solid border-gray-400 rounded-lg "
    ...
    <div className="text-black flex   flex-col items-start gap-1 justify-center ">
      ...
      <p className="text-[14px] font-semibold">City: {card.city}</p>
        ...
        <div className='flex flex-col items-start justify-center gap-5'>
          <h1 className='text-sm hover:text-red-600 transition-all cursor-pointer text-gray-600 flex items-center justify-center gap-1'><Eye className='size-5' /> <span>{card.views}</span></h1>
          <h2 className='text-sm hover:text-red-600 transition-all cursor-pointer text-gray-600 flex items-center justify-center gap-1'> <HeartHandshake className='size-5' /> <span>{card.Supporters }</span> </h2>
      </div>
   </div>
  </div>
  )
